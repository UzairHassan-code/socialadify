...
async def update_current_user_profile_text_and_email( 
    ...
    new_email: Optional[str] = Form(None) # Added new_email form field
):
    ...
    if new_email is not None: update_data_dict["new_email"] = new_email # Add to dict for UserUpdate schema
    ...
    except Exception as e: # Pydantic validation error for new_email format
    ...
    try:
        updated_user_db = await user_service.update_user_profile(
            ...
            # profile_picture_url is not handled by this endpoint
        )
    except HTTPException as http_exc: # Catch specific HTTPExceptions from CRUD (e.g., email already in use)
        raise http_exc
    ...
    if not updated_user_db:
        # This case should ideally be covered by exceptions from CRUD or if no data was to update
    ...
    # Return UserPublic, but the message about re-login is crucial for the frontend to handle
...
async def upload_profile_picture(
    ...
    # ... (profile picture upload logic remains the same)
