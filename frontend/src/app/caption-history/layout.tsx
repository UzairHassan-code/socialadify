// D:\socialadify\frontend\src\app\caption-history\layout.tsx
'use client';
...
// Re-using Icon components (ideally, from a shared file)
...
const SocialAdifyLogoIcon = ({ className = "w-8 h-8" }: {className?: string}) => ( <svg className={className} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"> <defs> <linearGradient id="socialAdifyLogoGradientHistoryLayout" x1="0%" y1="0%" x2="100%" y2="100%"> <stop offset="0%" style={{stopColor: 'rgb(56, 189, 248)'}} /> {/* sky-500 */} <stop offset="50%" style={{stopColor: 'rgb(99, 102, 241)'}} /> {/* indigo-500 */} <stop offset="100%" style={{stopColor: 'rgb(124, 58, 237)'}} /> {/* purple-600 */} </linearGradient> </defs> <circle cx="50" cy="50" r="45" fill="url(#socialAdifyLogoGradientHistoryLayout)" /> <path d="M35 65 L35 35 Q50 25 65 35 L65 65 Q50 75 35 65 Z M50 42 A8 8 0 0 1 50 58 A8 8 0 0 1 50 42 Z" fill="white" /> </svg> );
...
export default function CaptionHistoryLayout({
  children,
}: {
  children: ReactNode;
}) {
    ...
    <ProtectedRoute>
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 via-gray-800 to-slate-900 text-slate-100">
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16 md:h-20">
              <Link href="/home" className="flex items-center gap-3 group">
                <SocialAdifyLogoIcon className="h-8 w-8 sm:h-9 sm:w-9 transition-transform duration-300 group-hover:rotate-[7deg] group-hover:scale-105" />
                <span className="text-xl sm:text-2xl font-bold text-slate-50 group-hover:text-sky-300 transition-colors duration-300 tracking-tight">
                  SocialAdify
                </span>
              </Link>
              {isAuthReady && isAuthenticated && (
                <div className="flex items-center gap-3 sm:gap-4">
                  <Link 
                    ...
                    title="My Account"
                  >
                    <UserIcon className="h-6 w-6 sm:h-7 sm:w-7 text-slate-300 group-hover:text-sky-300 transition-colors" />
                    {user?.firstname && (
        ...
        <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          {children} {/* This will be your caption-history/page.tsx */}
        </main>
        <footer className="bg-slate-900/60 border-t border-slate-700/60 text-center py-6 mt-auto">
          <p className="text-xs text-slate-500">
            © {new Date().getFullYear()} SocialAdify. Your Caption Library.
          </p>
        </footer>
      </div>
    </ProtectedRoute>
