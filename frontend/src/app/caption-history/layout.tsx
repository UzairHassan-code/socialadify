...
const SocialAdifyLogoIcon = ({ className = "w-8 h-8" }: {className?: string}) => ( <svg className={className} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"> <defs> <linearGradient id="socialAdifyLogoGradientHistoryLayout" x1="0%" y1="0%" x2="100%" y2="100%"> <stop offset="0%" style={{stopColor: 'rgb(56, 189, 248)'}} /> <stop offset="50%" style={{stopColor: 'rgb(99, 102, 241)'}} /> <stop offset="100%" style={{stopColor: 'rgb(124, 58, 237)'}} /> </linearGradient> </defs> <circle cx="50" cy="50" r="45" fill="url(#socialAdifyLogoGradientHistoryLayout)" /> <path d="M35 65 L35 35 Q50 25 65 35 L65 65 Q50 75 35 65 Z M50 42 A8 8 0 0 1 50 58 A8 8 0 0 1 50 42 Z" fill="white" /> </svg> );
                  ...
                  >
                    {profilePicUrl ? (
                        <Image 
                            ...
                            width={40} 
                            height={40}
                            className="rounded-full object-cover ring-1 ring-slate-600 group-hover:ring-sky-300 transition-all"
                            onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                        />
                    ) : (
                        <UserIcon className="h-8 w-8 text-slate-300 group-hover:text-sky-300 transition-colors" /> 
                    )}
                    {user.firstname && (
