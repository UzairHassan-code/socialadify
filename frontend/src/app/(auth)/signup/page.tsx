...
// SVG Icons (keeping them as you provided them)
...
export default function SignupPage() {
    ...
    // Destructure loginWithToken from useAuth
    const { signup, loginWithToken, isLoading: authIsLoading, error: authError, isAuthenticated, isAuthReady, clearError } = useAuth();
    ...
    const handleGoogleLoginSuccess = async (response: any) => {
        ...
        try {
            ...
            if (!backendResponse.ok) {
                throw new Error(data.detail || 'Failed to authenticate with Google via backend.');
            }

            console.log('Backend authentication successful:', data);
            
            // HIGHLIGHTED CHANGE START
            // Call the new loginWithToken function from AuthContext
            if (data.access_token) {
                await loginWithToken(data.access_token); 
                // Redirection to /home will now be handled by the useEffect in AuthContext
                // that listens to isAuthenticated changes.
            } else {
                setFormError("Backend did not return an access token.");
            }
            // HIGHLIGHTED CHANGE END
        } catch (error: any) {
    ...
    // --- END NEW: Google Login Logic ---


    useEffect(() => {
        // This useEffect in signup/page.tsx will automatically handle redirection
        // after isAuthenticated changes in AuthContext (updated by login or loginWithToken)
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            // Manual signup: redirect to login page for traditional login
