// D:/socialadify/frontend/src/app/template-generator/layout.tsx
'use client';
...
const SocialAdifyLogoIcon = ({ className = "w-8 h-8" }: {className?: string}) => (
    <svg className={className} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
        <defs>
            <linearGradient id="socialAdifyLogoGradientTmplGenLayout" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <circle cx="50" cy="50" r="45" fill="url(#socialAdifyLogoGradientTmplGenLayout)" />
        <path d="M30 60 Q40 40 50 50 T70 40 M30 60 L35 70 M70 40 L65 30" stroke="rgba(255,255,255,0.8)" strokeWidth="5" fill="none" strokeLinecap="round"/>
    </svg>
);
...
export default function TemplateGeneratorLayout({
    ...
    return (
        <ProtectedRoute>
            <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 via-gray-900 to-sky-950 text-slate-100">
                        ...
                        <div className="flex items-center justify-between h-16 md:h-20">
                            <div className="flex items-center">
                                <Link href="/home" className="flex items-center gap-3 group">
                                    ...
                                    <span className="text-xl sm:text-2xl font-semibold text-slate-100 group-hover:text-sky-400 transition-colors duration-300 tracking-tight">
                                        SocialAdify
                                    </span>
                                </Link>
                            </div>
                                    ...
                                    >
                                        {(profilePicUrl && !profileImageError) ? (
                                            <Image
                                                ...
                                                width={40}
                                                height={40}
                                                style={{ height: 40, width: 'auto' }} // --- FIX: Added style for aspect ratio ---
                                                className="rounded-full object-cover ring-1 ring-slate-600 group-hover:ring-sky-400 transition-all"
                                                onError={() => setProfileImageError(true)}
                                            />
                                        ) : (
                                            <UserIcon className="h-8 w-8 text-slate-400 group-hover:text-sky-400 transition-colors" /> 
                                        )}
                                        {user.firstname && (
                ...
                <footer className="bg-slate-900/60 border-t border-slate-700/50 text-center py-6 mt-auto">
                    <p className="text-xs text-slate-500">
                        © {new Date().getFullYear()} SocialAdify. Template Generation Module.
                    </p>
                </footer>
            </div>
        </ProtectedRoute>
