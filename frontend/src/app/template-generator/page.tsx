...
export default function TemplateGeneratorPage() {
        ...
        if (token) {
            getTemplates(token)
                ...
                .catch((err: any) => {
                    ...
                    setError("Could not load templates. Please try refreshing the page.");
                });
        }
    }, [token]);
    ...
    useEffect(() => {
        if (pageState === 'editor' && selectedTemplate && canvasRef.current) {
            ...
            baseImage.onload = () => {
                ...
                selectedTemplate.editable_fields?.forEach((field: EditableField) => {
                    const text = templateFormValues[field.key] || '';
    ...
    }, [selectedTemplate, templateFormValues, pageState]);

    const handleTemplateInputChange = (key: string, value: string) => {
        setTemplateFormValues(prev => ({ ...prev, [key]: value }));
    };
    ...
    const handleTemplateSubmit = async (event: FormEvent) => {
        ...
        try {
            const response = await generateFromTemplate(token, selectedTemplate.id, templateFormValues);
            setGeneratedPost({
    ...
    const handleSelectTemplate = (template: Template) => {
        ...
        const initialValues = template.editable_fields?.reduce((acc: Record<string, string>, field: EditableField) => {
            acc[field.key] = '';
            return acc;
        }, {}) || {};
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <header className="text-center mb-10">
                ...
                <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-teal-300 via-cyan-400 to-sky-400 bg-clip-text text-transparent pb-2">
                    Generate from Template
                </h1>
                <p className="mt-3 text-md text-slate-400 max-w-2xl mx-auto">
                    Choose a pre-designed template, fill in the details, and create your post instantly.
                </p>
            </header>
                     ...
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                        ...
                        <div className={cardClass}>
                            ...
                            <form onSubmit={handleTemplateSubmit} className="space-y-4">
                                {selectedTemplate.editable_fields?.map((field: EditableField) => (
                                    <div key={`${selectedTemplate.id}-${field.key}`}>
                                        <label htmlFor={field.key} className={labelClass}>{field.label}</label>
                                        <input
                                            type="text" id={field.key} value={templateFormValues[field.key] || ''}
