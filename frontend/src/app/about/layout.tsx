// D:\socialadify\frontend\src\app\about\layout.tsx
'use client';
...
const SocialAdifyLogoIcon = ({ className = "w-8 h-8" }: {className?: string}) => ( <svg className={className} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"> <defs> <linearGradient id="socialAdifyLogoGradientAboutLayout" x1="0%" y1="0%" x2="100%" y2="100%"> <stop offset="0%" style={{stopColor: 'rgb(168, 85, 247)'}} /> {/* purple-500 */} <stop offset="50%" style={{stopColor: 'rgb(236, 72, 153)'}} /> {/* pink-500 */} <stop offset="100%" style={{stopColor: 'rgb(244, 114, 182)'}} /> {/* rose-400 */} </linearGradient> </defs> <circle cx="50" cy="50" r="45" fill="url(#socialAdifyLogoGradientAboutLayout)" /> <path d="M30 60 Q40 40 50 50 T70 40 M30 60 L35 70 M70 40 L65 30" stroke="rgba(255,255,255,0.8)" strokeWidth="5" fill="none" strokeLinecap="round"/> </svg> );
...
export default function AboutLayout({
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 via-gray-900 to-purple-950 text-slate-100">
        <header className="bg-slate-900/80 backdrop-blur-xl shadow-2xl sticky top-0 z-50 border-b border-slate-700/50">
            ...
            <div className="flex items-center justify-between h-16 md:h-20">
              <div className="flex items-center">
                <Link href="/home" className="flex items-center gap-3 group">
                  ...
                  <span className="text-xl sm:text-2xl font-semibold text-slate-100 group-hover:text-purple-400 transition-colors duration-300 tracking-tight">
                    SocialAdify
                  </span>
                </Link>
              ...
              {isAuthReady && isAuthenticated && (
                <div className="flex items-center gap-2 sm:gap-3">
                  <Link 
                    ...
                    title="My Account"
                  >
                    <UserIcon className="h-5 w-5 sm:h-6 sm:w-6 text-slate-400 group-hover:text-purple-400 transition-colors" />
                    {user?.firstname && (
        ...
        <footer className="bg-slate-900/60 border-t border-slate-700/50 text-center py-6 mt-auto">
          <p className="text-xs text-slate-500">
            © {new Date().getFullYear()} SocialAdify. About Our Mission.
          </p>
        </footer>
      </div>
    </ProtectedRoute>
