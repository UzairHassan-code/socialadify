...
export default function AccountLayout({
  ...
  console.log("AccountLayout: Rendering with new theme. ProtectedRoute will apply.");
            ...
            <div className="flex items-center justify-between h-16 md:h-20">
              ...
              {isAuthReady && isAuthenticated && (
                <div className="flex items-center gap-3 sm:gap-4">
                  <Link 
                    href="/account" 
                    className="flex items-center p-2 rounded-lg hover:bg-slate-700/60 transition-colors group"
                    title="My Account"
                  >
                    <UserIcon className="h-6 w-6 sm:h-7 sm:w-7 text-slate-300 group-hover:text-indigo-300 transition-colors" />
                    {user?.firstname && (
