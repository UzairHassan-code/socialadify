...
import {
    ...
    fetchScheduledPosts,
    deleteScheduledPost, 
    // updateScheduledPost, // Service function is imported, but modal handles call
    ScheduledPost,
    // ScheduledPostPayload 
} from '@/services/schedulerService';
import Link from 'next/link'; 
import EditScheduledPostModal from '../../components/EditScheduledPostModal'; // Import the new modal
...
export default function SchedulerPage() {
    ...
    const loadScheduledPosts = useCallback(async (page: number, reset: boolean = false) => {
        // ... (existing loadScheduledPosts logic) ...
        if (!token) { setIsFetchingPosts(false); return; }
        setIsFetchingPosts(true); setError(null);
        try {
            const posts = await fetchScheduledPosts(token, page * ITEMS_PER_PAGE, ITEMS_PER_PAGE);
            if (reset || page === 0) setScheduledPosts(posts);
            else setScheduledPosts(prev => [...prev, ...posts]);
    ...
    useEffect(() => {
        ...
        if (isAuthReady && isAuthenticated) { loadScheduledPosts(0, true); }
        else if (isAuthReady && !isAuthenticated) { setError("Please log in to manage scheduled posts."); setIsFetchingPosts(false); }
    }, [isAuthReady, isAuthenticated, loadScheduledPosts]);

    const handleImageChange = (event: ChangeEvent<HTMLInputElement>) => {
        // ... (existing image change logic) ...
        const file = event.target.files?.[0]; setError(null); 
        if (file) {
            if (file.size > 10 * 1024 * 1024) { setError("Image size should not exceed 10MB."); setImageFile(null); setImagePreviewUrl(null); return; }
            if (!file.type.startsWith("image/")) { setError("Invalid file type. Please upload an image."); setImageFile(null); setImagePreviewUrl(null); return; }
            setImageFile(file); const reader = new FileReader(); reader.onloadend = () => { setImagePreviewUrl(reader.result as string); }; reader.readAsDataURL(file);
        } else { setImageFile(null); setImagePreviewUrl(null); }
    };

    const handleSchedulePost = async (event: FormEvent) => {
        // ... (existing schedule post logic) ...
        event.preventDefault();
        if (!token || !imageFile || !caption || !scheduledDateTime) { setError("Image, caption, and schedule date/time are required."); return; }
        ...
        try {
            ...
            setCaption(''); setImageFile(null); setImagePreviewUrl(null); setScheduledDateTime(''); setTargetPlatform('');
            setCurrentPage(0); loadScheduledPosts(0, true); 
            setTimeout(() => setSuccessMessage(null), 3000);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : "Failed to schedule post."; setError(errorMessage);
            if (errorMessage.toLowerCase().includes("unauthorized")) logout();
    ...
    const handleDeletePost = async (postId: string) => {
        ...
        if (!token) { setError("Authentication error. Please log in again."); logout(); return; }
        if (!window.confirm("Are you sure you want to delete this scheduled post? This action cannot be undone.")) return;
        setDeletingPostId(postId); setError(null); setSuccessMessage(null);
        try {
            await deleteScheduledPost(token, postId);
            setSuccessMessage("Post deleted successfully!");
            setScheduledPosts(prevPosts => prevPosts.filter(p => p.id !== postId));
            setTimeout(() => setSuccessMessage(null), 3000);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : "Failed to delete post."; setError(errorMessage);
            if (errorMessage.toLowerCase().includes("unauthorized")) logout();
    ...
    const formatDate = (dateString: string) => { /* ... */ return new Date(dateString).toLocaleString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }); };
    ...
    const buttonPrimaryClass = "w-full flex items-center justify-center px-6 py-3 text-sm font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 transition duration-150 ease-in-out disabled:opacity-60 disabled:cursor-not-allowed bg-orange-600 hover:bg-orange-500 text-white";
    const cardBaseClass = "bg-slate-800/70 backdrop-blur-xl rounded-2xl shadow-2xl p-6 sm:p-8 border border-slate-700/80";

    if (!isAuthReady) { /* ... */ return <div className="flex items-center justify-center min-h-[calc(100vh-200px)]"><LoadingSpinner className="w-10 h-10 text-orange-400"/></div>; }
    if (!isAuthenticated && isAuthReady) { /* ... */  return <div className="text-center py-10 text-slate-400">Please <Link href="/login" className="text-orange-400 hover:underline">log in</Link> to use the scheduler.</div>; }
        ...
        <>
            <div className="space-y-10">
                ...
                <header className="text-center">
                    <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-amber-400 via-orange-500 to-red-500 bg-clip-text text-transparent pb-2">Post Scheduler</h1>
                                ...
                                <div key={post.id} className="bg-slate-700/50 p-4 rounded-lg shadow-md flex flex-col sm:flex-row gap-4 items-start border border-slate-600/70">
                                    ...
                                    <div className="flex sm:flex-col gap-2 mt-2 sm:mt-0 flex-shrink-0">
                                        <button 
                                            ...
                                            disabled={deletingPostId === post.id} // Disable if deleting
                                        >
