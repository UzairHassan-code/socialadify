...
import { useImageContext } from '@/context/ImageContext'; // *** NEW: Import the ImageContext hook ***
...
const AlertTriangleIcon = ({ className = "w-5 h-5 text-yellow-400" }: {className?: string}) => ( <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" ><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg> );
...
export default function CaptionGeneratorPage() {
    ...
    const { sharedImage, setSharedImage } = useImageContext(); // *** NEW: Get state from context ***
    ...
    // *** NEW: useEffect to load the shared image from context ***
    useEffect(() => {
        if (sharedImage) {
            ...
            // Clear the image from the context so it's not used again
            setSharedImage(null);
        }
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-slate-300 relative overflow-hidden">
            ...
            <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 pt-24">
                <header className="text-center relative mb-10"> 
                    <div className="flex justify-between items-center mb-4 px-2 sm:px-0">
                        ...
                        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white tracking-tight pb-1 flex-grow text-center">
                ...
                <section className={`${cardBaseClass} max-w-2xl mx-auto mb-10`}>
                    ...
                    <label htmlFor="imageUpload" className="mt-1 flex flex-col items-center justify-center w-full h-64 px-6 pt-5 pb-6 border-2 border-slate-600/80 border-dashed rounded-xl group hover:border-blue-500 transition-colors bg-slate-700/30 cursor-pointer">
                        {imagePreviewUrl ? (
                            ...
                            <div className="space-y-2 text-center">
                                <UploadIcon className="w-10 h-10 text-slate-500 group-hover:text-blue-400 transition-colors" />
                                <div className="flex text-sm text-slate-400 group-hover:text-blue-300 transition-colors">
                                    <span className="font-medium text-blue-400 group-hover:text-blue-300">Upload a file</span>
                    ...
                    <form onSubmit={handleGenerateCaptions} className="space-y-5">
                        ...
                        <fieldset className="space-y-3 pt-2">
                            ...
                            <div className="relative flex items-start">
                                <div className="flex h-5 items-center"><input id="includeHashtags" type="checkbox" checked={includeHashtags} onChange={(e) => setIncludeHashtags(e.target.checked)} className="h-4 w-4 rounded border-slate-500 text-blue-500 focus:ring-blue-400 bg-slate-600" /></div>
                                <div className="ml-3 text-sm"><label htmlFor="includeHashtags" className="font-medium text-slate-200">Hashtags</label></div>
                            </div>
                            <div className="relative flex items-start">
                                <div className="flex h-5 items-center"><input id="includeEmojis" type="checkbox" checked={includeEmojis} onChange={(e) => setIncludeEmojis(e.target.checked)} className="h-4 w-4 rounded border-slate-500 text-blue-500 focus:ring-blue-400 bg-slate-600" /></div>
                ...
                {(generatedCaptions.length > 0 || isLoading || infoMessage) && (
                    <section className={`${cardBaseClass} max-w-3xl mx-auto`}>
                        ...
                        {infoMessage && !isLoading && (
                               <div className="p-3 mb-4 text-sm text-slate-300 bg-slate-700/40 border border-slate-600/60 rounded-md flex items-center gap-2">
                                <AlertTriangleIcon className="w-5 h-5 text-slate-300 flex-shrink-0" />
                                {infoMessage}
                            </div>
                        )}
                        ...
                        {!isLoading && generatedCaptions.length > 0 && (
                            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                                    ...
                                    <div key={captionItem.id} className="p-4 bg-slate-700/70 rounded-lg border border-slate-600/90 shadow-lg">
                                        {captionItem.isEditing ? (
                                            <textarea
                                                ...
                                                className="w-full p-2 text-sm border border-blue-500 rounded-md bg-slate-600 text-slate-100 focus:ring-1 focus:ring-blue-400 outline-none min-h-[80px]"
                                                rows={3}
                                            />
                                        ) : (
                                        ...
                                        <div className="flex justify-end items-center space-x-2.5 mt-2 border-t border-slate-600/70 pt-2.5">
                                            {captionItem.isEditing ? (
                                                ...
                                                <button 
                                                    ...
                                                    className={`${buttonSecondaryClass} bg-slate-600/60 hover:bg-slate-500/60 text-slate-300 hover:text-slate-100`}
                                                >
                                                    <EditIcon/>
                                                </button>
                                            )}
                                            ...
                                            <button
                                                ...
                                                className={`${buttonSecondaryClass} ${captionItem.isSaved ? 'bg-emerald-600/80 text-white cursor-default' : 'bg-slate-500 hover:bg-slate-600 text-white'} disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5`}
                                            >
                                ...
                                <div className="pt-4 text-center">
                                    <button onClick={handleGenerateCaptions} disabled={isLoading || !isAuthenticated} className={`${buttonPrimaryClass} inline-flex items-center gap-2`}>
