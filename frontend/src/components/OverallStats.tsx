...
// Make sure to add the new fields to this interface in `insightsService.ts` as well.
...
// --- SVG Icons (Originals + 5 New Ones) ---
...
const IconConversions = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 mr-1.5 text-gray-400"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
...
// --- EXPANDED METRIC_DISPLAY_CONFIG ---
const METRIC_DISPLAY_CONFIG: { 
    [key in keyof Statistics]?: { 
        label: string; 
        icon?: React.FC;
...
// --- EXPANDED list of metrics to show, in desired order ---
const metricsToShow: (keyof Statistics)[] = [
    'impressions', 'clicks', 'reach', 'ctr', 'conversions', 
    'cost', 'avg_cpc', 'cpm', 'cpa', 'frequency'
];
...
export default function OverallStats({ data, title = "Detailed Statistics", isLoading = false }: OverallStatsProps) {
    ...
    return (
        <section className="bg-white p-4 md:p-6 rounded-xl shadow-lg">
            <h2 className="text-xl md:text-2xl font-semibold text-gray-700 mb-6">{title}</h2>
            {/* MODIFIED: Grid layout now supports more items gracefully */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-5">
...
// NOTE: You will also need to update the `Statistics` interface in your `insightsService.ts`
// file to include the new fields: reach, frequency, cpm, conversions, cpa.
